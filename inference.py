...
def draw_results(image, detection_result, lstm_output):
...
# Load in LSTM binary model for ZoomIn/ZoomOut

# Define an array to store the hand key point sequence
# TODO: Replace with constants later
gesture_seq = np.zeros((21, 3, 10))
...
with mp_hands.Hands(
    ...
    while cap.isOpened():
        ...
        # Every 3 s / 10 frames = 0.3s, record the 21 key points to `gesture_seq` 

        # If gesture_seq is full (all 10 entries acquired, compute LSTM result 

        # Draw the hand annotations on the image.
        image.flags.writeable = True
        image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
        processed = draw_results(image, results, None)
